...
# 84
return(unsigned)cf?({struct Cyc___cycFILE*_tmp5=_cycalloc(sizeof(*_tmp5));_tmp5->file=cf;_tmp5;}): 0;}
# 88
...
# 102
unsigned long fread(char*,unsigned long,unsigned long,struct __abstractFILE*);static char _tmp9[27U]="fread: buffer insufficient";
# 104
static struct Cyc_Core_Failure_exn_struct Cyc___fread_failure={Cyc_Core_Failure,{_tmp9,_tmp9,_tmp9 + 27U}};
# 106
unsigned long Cyc_fread(struct _fat_ptr ptr,unsigned long size,unsigned long nmemb,struct Cyc___cycFILE*f){
if(size * nmemb > _get_fat_size(ptr,sizeof(char)))(int)_throw(& Cyc___fread_failure);return({fread((char*)_untag_fat_ptr(ptr,sizeof(char),1U),size,nmemb,f->file);});}
# 112
unsigned long fwrite(const char*,unsigned long,unsigned long,struct __abstractFILE*);static char _tmpB[31U]="fwrite called with NULL string";
# 114
static struct Cyc_Core_Failure_exn_struct Cyc___fwrite_failure_1={Cyc_Core_Failure,{_tmpB,_tmpB,_tmpB + 31U}};static char _tmpC[28U]="fwrite: buffer insufficient";
# 116
static struct Cyc_Core_Failure_exn_struct Cyc___fwrite_failure_2={Cyc_Core_Failure,{_tmpC,_tmpC,_tmpC + 28U}};
# 118
unsigned long Cyc_fwrite(struct _fat_ptr ptr,unsigned long size,unsigned long nmemb,struct Cyc___cycFILE*f){
if(!((unsigned)ptr.curr))(int)_throw(& Cyc___fwrite_failure_1);else{
# 121
if(size * nmemb > _get_fat_size(ptr,sizeof(char)))(int)_throw(& Cyc___fwrite_failure_2);return({fwrite((const char*)_untag_fat_ptr(ptr,sizeof(char),1U),size,nmemb,f->file);});}}
# 127
...
struct Cyc___cycFILE*f=({({const char*_tmp1C=(const char*)_check_null(_untag_fat_ptr(fname,sizeof(char),1U));Cyc_fopen(_tmp1C,(const char*)_check_null(_untag_fat_ptr(mode,sizeof(char),1U)));});});
if(f == 0){
struct _fat_ptr fn=({unsigned _tmp15=_get_fat_size(fname,sizeof(char))+ 1U;char*_tmp14=_cycalloc_atomic(_check_times(_tmp15,sizeof(char)));({{unsigned _tmp1B=_get_fat_size(fname,sizeof(char));unsigned i;for(i=0;i < _tmp1B;++ i){_tmp14[i]=((const char*)fname.curr)[(int)i];}_tmp14[_tmp1B]=0;}0;});_tag_fat(_tmp14,sizeof(char),_tmp15);});
(int)_throw(({struct Cyc_FileOpenError_exn_struct*_tmp13=_cycalloc(sizeof(*_tmp13));_tmp13->tag=Cyc_FileOpenError,_tmp13->f1=fn;_tmp13;}));}
# 168
return f;}
# 166
...
if(({Cyc_fclose(f);})!= 0)(int)_throw(& Cyc_FileCloseError_val);}
# 183
char*getcwd(char*buf,unsigned long size);static char _tmp18[29U]="getcwd: invalid buf argument";
# 185
static struct Cyc_Core_Failure_exn_struct Cyc___getcwd_failure={Cyc_Core_Failure,{_tmp18,_tmp18,_tmp18 + 29U}};
# 187
struct _fat_ptr Cyc_getcwd(struct _fat_ptr buf,unsigned long size){
if(_get_fat_size(buf,sizeof(char))< size)(int)_throw(& Cyc___getcwd_failure);{char*response=({getcwd((char*)_untag_fat_ptr(buf,sizeof(char),1U),size);});
# 190
