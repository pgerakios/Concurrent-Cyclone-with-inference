...
if(_get_fat_size(Cyc_Lexing_aux_buffer,sizeof(char))== (unsigned)1)Cyc_Lexing_aux_buffer=({Cyc_Core_new_string((unsigned)(4096 + 1));});{int read=({(((struct Cyc_Lexing_function_lexbuf_state*)lexbuf->refill_state)->read_fun)(Cyc_Lexing_aux_buffer,(int)(_get_fat_size(Cyc_Lexing_aux_buffer,sizeof(char))- (unsigned)1),((struct Cyc_Lexing_function_lexbuf_state*)lexbuf->refill_state)->read_fun_state);});
# 82
...
({({struct _fat_ptr _tmp14=({struct _fat_ptr _tmp12=_fat_ptr_decrease_size(newbuf,sizeof(char),1U);_fat_ptr_plus(_tmp12,sizeof(char),oldlen);});struct _fat_ptr _tmp13=(struct _fat_ptr)lexbuf->lex_buffer;Cyc_zstrncpy(_tmp14,_tmp13,(unsigned long)oldlen);});});
...
# 83
({({struct _fat_ptr _tmp16=
# 96
_fat_ptr_decrease_size(lexbuf->lex_buffer,sizeof(char),1U);struct _fat_ptr _tmp15=(struct _fat_ptr)
_fat_ptr_plus(lexbuf->lex_buffer,sizeof(char),n);Cyc_zstrncpy(_tmp16,_tmp15,(unsigned long)(lexbuf->lex_buffer_len - n));});});
# 99
({({struct _fat_ptr _tmp18=_fat_ptr_decrease_size(_fat_ptr_plus(lexbuf->lex_buffer,sizeof(char),lexbuf->lex_buffer_len - n),sizeof(char),1U);struct _fat_ptr _tmp17=(struct _fat_ptr)Cyc_Lexing_aux_buffer;Cyc_zstrncpy(_tmp18,_tmp17,(unsigned long)n);});});
# 101
...
return({struct Cyc_Lexing_lexbuf*_tmp2=_cycalloc(sizeof(*_tmp2));_tmp2->refill_buff=Cyc_Lexing_lex_refill,({
struct Cyc_Lexing_function_lexbuf_state*_tmp1A=({struct Cyc_Lexing_function_lexbuf_state*_tmp1=_cycalloc(sizeof(*_tmp1));_tmp1->read_fun=read_fun,_tmp1->read_fun_state=read_fun_state;_tmp1;});_tmp2->refill_state=_tmp1A;}),({
struct _fat_ptr _tmp19=({Cyc_Core_new_string((unsigned)(8192 + 1));});_tmp2->lex_buffer=_tmp19;}),_tmp2->lex_buffer_len=8192,_tmp2->lex_abs_pos=- 8192,_tmp2->lex_start_pos=8192,_tmp2->lex_curr_pos=8192,_tmp2->lex_last_pos=8192,_tmp2->lex_last_action=0,_tmp2->lex_eof_reached=0;_tmp2;});}
# 118
...
return({({struct Cyc_Lexing_lexbuf*(*_tmp1B)(int(*read_fun)(struct _fat_ptr,int,struct Cyc___cycFILE*),struct Cyc___cycFILE*read_fun_state)=({struct Cyc_Lexing_lexbuf*(*_tmp5)(int(*read_fun)(struct _fat_ptr,int,struct Cyc___cycFILE*),struct Cyc___cycFILE*read_fun_state)=(struct Cyc_Lexing_lexbuf*(*)(int(*read_fun)(struct _fat_ptr,int,struct Cyc___cycFILE*),struct Cyc___cycFILE*read_fun_state))Cyc_Lexing_from_function;_tmp5;});_tmp1B(Cyc_Lexing_read_from_file,f);});});}
# 127
...
return({struct Cyc_Lexing_lexbuf*_tmp8=_cycalloc(sizeof(*_tmp8));_tmp8->refill_buff=Cyc_Lexing_set_eof,_tmp8->refill_state=(void*)0,({
# 134
struct _fat_ptr _tmp1C=({Cyc_strdup((struct _fat_ptr)s);});_tmp8->lex_buffer=_tmp1C;}),_tmp8->lex_buffer_len=(int)
_get_fat_size(s,sizeof(char)),_tmp8->lex_abs_pos=0,_tmp8->lex_start_pos=0,_tmp8->lex_curr_pos=0,_tmp8->lex_last_pos=0,_tmp8->lex_last_action=0,_tmp8->lex_eof_reached=1;_tmp8;});}
# 140
...
({({struct _fat_ptr _tmp1E=_fat_ptr_decrease_size(s,sizeof(char),1U);struct _fat_ptr _tmp1D=(struct _fat_ptr)_fat_ptr_plus(lbuf->lex_buffer,sizeof(char),lbuf->lex_start_pos);Cyc_zstrncpy(_tmp1E,_tmp1D,(unsigned long)len);});});
({struct _fat_ptr _tmpA=_fat_ptr_plus(s,sizeof(char),len);char _tmpB=*((char*)_check_fat_subscript(_tmpA,sizeof(char),0U));char _tmpC='\000';if(_get_fat_size(_tmpA,sizeof(char))== 1U &&(_tmpB == 0 && _tmpC != 0))_throw_arraybounds();*((char*)_tmpA.curr)=_tmpC;});
return s;}
# 140
