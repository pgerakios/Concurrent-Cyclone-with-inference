...
#ifdef __RUNTIME_MEMORY_DEBUG_XLSTEAL
 #define dbgmsg_xlsteal(arg...)  { fprintf(stderr,##arg); fflush(stderr);} 
#else
 #define dbgmsg_xlsteal(arg...)  {} 
#endif

#ifdef __RUNTIME_MEMORY_DEBUG_XLINC
 #define dbgmsg_xlinc(arg...)  { fprintf(stderr,##arg); fflush(stderr);} 
#else
 #define dbgmsg_xlinc(arg...)  {} 
#endif

#ifdef __RUNTIME_MEMORY_DEBUG_XLDEC
 #define dbgmsg_xldec(arg...)  { fprintf(stderr,##arg); fflush(stderr);} 
#else
 #define dbgmsg_xldec(arg...)  {} 
#endif
...
{	//ret.sub_regions = 0;
   //	ret.parent = (parent == (struct _XRegionHandle *) CYC_CORE_HEAP_REGION)?NULL:parent;
	...
	ret.last_plus_one = 0;
	ret.key  = 0;
	/* Step 1.2: initialize global key cache (used for thread-local rg)*/
	...
	z = (struct _XRegionHandle *) 
        ...
        assert(z > 100);
	*z = ret; 
	  dbgmsg_new_xregion("\n[%X] _new_xregion: idx=%d xregion_global_handle=%p"
   ...
   /* Step 2.1: find parent tkey and register this key to the parent*/
	unsigned int wr_par,rd_par; // implicit ancestor lock counts 
...
void * _xregion_malloc(struct _XRegionHandle *r,
   	    	       unsigned int s) {
  unsigned int shared = r->caps.rcnt > 1 ;
#ifndef CYC_NOALIGN
    // round s up to the nearest _CYC_MIN_ALIGNMENT value
    s =  (s + _CYC_MIN_ALIGNMENT - 1) & (~(_CYC_MIN_ALIGNMENT - 1));
#endif
  ...
  if(s > (r->last_plus_one - r->offset)) {
    unsigned int next_size = !r->offset ?
                      ...
                      ((unsigned int) 
                      ((long long) r->last_plus_one - (long long) (r->curr + 1)))*2;
...
static void __xregion_lockop(struct _XTreeHandle *tk,
 ...
 const unsigned int self = tcb()->tid; 
   ...
   for( i=0,child = tk->sub_keys ; child != NULL ; 
       ...
       dbgmsg_xldec("\n[%X] __xregion_locop LOCK STEP1.%d: %p "
                    " (tree_key=%p)\n",
                     self,i,tk,child);
 ...
 unsigned int *rd_cnt = &h->caps.rd_cnt;
 unsigned short *local_wr_cnt = &tk->caps.wr_cnt;
 unsigned short *local_rd_cnt = &tk->caps.rd_cnt;
 int wait_wq = 0;
 int wait_rq = 0;
 assert(write_lock > 0 || *local_wr_cnt >= -write_lock);
 assert(read_lock > 0  || *local_rd_cnt >= -read_lock);
 assert( *wr_cv != self ||
         (*wr_cv == self &&
            *local_wr_cnt > 0 &&
  ...
  do { 
   // wait if necessary
    if( wait_wq) { 
      __sync_fetch_and_add(wr_q,1);//move to write q
      while(*wr_cv != 0)
       futex_wait((int *) wr_cv,(int)*wr_cv);
      __sync_fetch_and_add(wr_q,-1);//remove from write q
    } else if(wait_rq) {
      __sync_fetch_and_add(rd_q,1);//move to read q
      rd_q[0]++;
      while(*rd_cv != 0)
        futex_wait((int *) rd_cv,(int) *rd_cv);
...
static int __xregion_refcnt(struct _XTreeHandle *tk,
   ...
   assert(*rcnt);
   assert(*rcnt > 0 && (rg > 0 || *rcnt >= -rg));
   *rcnt +=rg;
   if(rcnt[0]) return 1;
   hremove(tk->h);
   struct _XTreeHandle *iter,*prev;
   //1: remove handle "tk" from parent
...
void print_xtree( struct _XTreeHandle *tk, const unsigned int depth){
 ...
 for( i = 0 ; i < depth ; i++ ) printf("\t");
   printf("- Node = %p Rg = %d Wr_Lk = %d Rd_Lk = %d"
