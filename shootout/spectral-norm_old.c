...
volatile int next = 1;
static __thread mytid  = 0;
int gettid() {
  return mytid?mytid:__sync_fetch_and_add(&next,2);
}
...
eval_At_times_u(struct Param *p)
{
    ...
    for (i = p->r_begin, ie = p->r_end; i < ie; i++)
    {
...

        {
            ...
            p->v[i] = mem[0] + mem[1]; // WRITE V
        }

        /* odd size array  READ TMP, WRITE V*/
        for (j = j*2; __builtin_expect(j < p->N, false); j++)
