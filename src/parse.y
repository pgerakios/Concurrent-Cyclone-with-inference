...
%token NEW ABSTRACT FALLTHRU CAP USING NAMESPACE DATATYPE
%token SPAWN MALLOC RMALLOC RMALLOC_INLINE CALLOC RCALLOC SWAP
...
%token FAT_QUAL NOTNULL_QUAL NULLABLE_QUAL REQUIRES_QUAL ENSURES_QUAL
		 IEFFECT_QUAL OEFFECT_QUAL THROWS_QUAL NOTHROW_QUAL THROWSANY_QUAL
		 REENTRANT_QUAL XRGN_QUAL
...
%type <stmt_t> cap_statement spawn_statement
...
%type <kind_t> kind
%type <type_t> any_type_name type_var eff_opt //rgn_opt 
...
%type<type_opt_t>  ieffect_clause_opt oeffect_clause_opt
%type<throws_t> throws_clause_opt
%type<reentrant_t> reentrant_clause_opt
%type<list_t<type_t,`H>> eff_list
/* start production */
%start prog
%%
...
external_declaration:
  function_definition 
 {		let d = new_decl(new Fn_d($1),SLOC(@1));
		$$=^$(new List(d,NULL));}
| declaration         {$$=$!1;}
| error               {$$=^$(NULL);}
;
...
/* region h = open(k); or region h = xopen(k) */
| REGION IDENTIFIER '=' IDENTIFIER '(' expression ')' ';'
  { let two = $2;
    ...
    let six = $6;
	 ...
	 if( !ba && !bb )
			Warn::err(SLOC(@4),"expecting `open' or `xopen'");
	 if(ba){
    	tvar_t tv = new Tvar(new (string_t)aprintf("`%s",two), -1,
			 Tcutil::kind_to_bound(&Tcutil::rk));
	    type_t t = var_type(tv);
...
| REGION IDENTIFIER '@'  expression  ';'
  { let two = $2;
    ...
    vardecl_t vd = new_vardecl(SLOC(@3), // h : rgn(\rho). fresh \rho
										 new $(Loc_n,new two),
										 rgn_handle_type(t),
										 NULL);
    ...
    $$ = ^$(new List(region_decl(tv,vd,four,LOC(@1,@8)),NULL));
  }
;
...
attribute:
  IDENTIFIER { $$ = ^$(parse_nullary_att(SLOC(@1),$1)); }
| CONST      { $$ = ^$(&Const_att_val); } // because const a keyword!
| IDENTIFIER '(' assignment_expression ')'
             { $$ = ^$(parse_unary_att(SLOC(@1), $1, SLOC(@3), $3)); }
| IDENTIFIER '(' IDENTIFIER ',' INTEGER_CONSTANT ',' INTEGER_CONSTANT ')'
             { $$ = ^$(parse_format_att(LOC(@1,@8), SLOC(@3), $1, $3,
					      cnst2uint(SLOC(@5),$5),
					      cnst2uint(SLOC(@7),$7))); }
;
/*
  ...
  { let s = $1;
    ...
    if (zstrcmp(s,"aligned") == 0 || zstrcmp(s,"__aligned__") == 0) {
      ...
      if (zstrcmp(s,"regparm") == 0 || zstrcmp(s,"__regparm__") == 0) {
        ...
        Warn::err(SLOC(@1),aprintf("unrecognized attribute 2: %s",s));
        a = &Cdecl_att_val;
      }
    }
    $$=^$(a);
  ...
  { let s = $1; 
    ...
    if (zstrcmp(s,"format") == 0 || zstrcmp(s,"__format__") == 0)
      if (zstrcmp(t,"printf") == 0 || zstrcmp(t,"__printf__") == 0)
        ...
        Warn::err(SLOC(@3),aprintf("unrecognized format type 3 : %s",s)); 
    else 
      Warn::err(LOC(@1,@8),aprintf("unrecognized attribute 4 : %s",s)); 
    $$ = ^$(a);
  }
;
*/
...
/* Cyc: new */
kind:
  field_name { $$=^$(id_to_kind($1,SLOC(@1))); }
;
...
eff_list:
	'{'	type_name_list '}' 
{
    $$=^$(new List(join_eff($2),NULL));
}
|
eff_list ','	'{'	type_name_list '}' 
{
  $$=^$(new List(join_eff($4),$1));
}
;

ieffect_clause_opt:
   XRGN_QUAL  /* '(' type_name_list ')' */
    {
		 $$=^$(void_type);
    }
 | /* empty */ { $$ = ^$(NULL); }
...
throws_clause_opt:
  THROWS_QUAL '(' datatypefield_list')'
	 {
		 $$=^$($3);
	 }
 | NOTHROW_QUAL
...
| direct_declarator '(' parameter_type_list ')' requires_clause_opt
	 ensures_clause_opt ieffect_clause_opt  oeffect_clause_opt 
	 throws_clause_opt
	 reentrant_clause_opt
    { let &$(lis,b,c,eff,po) = $3;
      let req = $5;
      let ens = $6;
      let ieff = Absyn::parse_rgneffects(SLOC(@1),$7);
      let oeff = Absyn::parse_rgneffects(SLOC(@1),$8);
      let throws = $9;
      $$=^$(Declarator($1.id, $1.varloc,rnew(yyr) List(rnew(yyr) Function_mod(rnew(yyr) WithTypes(lis,b,c,eff,po,req,ens,ieff,oeff,throws,$10)),$1.tms)));
    }
| direct_declarator '(' optional_effect optional_rgn_order ')'
	 requires_clause_opt ensures_clause_opt  
	 ieffect_clause_opt  oeffect_clause_opt 
	 throws_clause_opt
	 reentrant_clause_opt
    {
		let ieff = Absyn::parse_rgneffects(SLOC(@1),$8);
		let oeff = Absyn::parse_rgneffects(SLOC(@1),$9);
		let throws = $10;
		 $$=^$(Declarator($1.id, $1.varloc,
...
| direct_declarator '(' identifier_list ')'
    { $$=^$(Declarator($1.id, $1.varloc, rnew(yyr) List(rnew(yyr) 
			Function_mod(rnew(yyr) NoTypes($3,LOC(@1,@4))),$1.tms))); }
    ...
    { let &$(lis,b,c,eff,po) = $3;
      ...
      $$=^$(Declarator(one.id, one.varloc, 
					...
					Function_mod(rnew(yyr)
					 WithTypes(lis,b,c,eff,po,req,ens,ieff,oeff,throws,$10)),
					 one.tms)));
    }
...
| REGION_QUAL '('  eff_set ')' /*any_type_name*/
...
| '(' optional_effect optional_rgn_order ')'
	 	 ...
	 	 reentrant_clause_opt
    {
		...
		let oeff = Absyn::parse_rgneffects(SLOC(@1),$8);
		let throws = $9;
		 $$=^$(Abstractdeclarator(rnew(yyr) List(rnew(yyr) 
										   Function_mod(rnew(yyr) 
											WithTypes(NULL,false,NULL,$2,$3,$5,$6,
											ieff,oeff,throws,$10)),NULL)));
    }
| '(' parameter_type_list ')' requires_clause_opt ensures_clause_opt
	 ieffect_clause_opt  oeffect_clause_opt  throws_clause_opt
	 reentrant_clause_opt

    { let &$(lis,b,c,eff,po) = $2;
		let ieff = Absyn::parse_rgneffects(SLOC(@1),$6);
		let oeff = Absyn::parse_rgneffects(SLOC(@1),$7);
		let throws = $8;
      $$=^$(Abstractdeclarator(rnew(yyr) List(rnew(yyr) 
											Function_mod(rnew(yyr) 
											WithTypes(lis,b,c,eff,po,$4,$5,
															ieff,oeff,throws,$9)),NULL)));
    }
...
| direct_abstract_declarator '(' parameter_type_list ')' 
    ...
    { let &$(lis,b,c,eff,po) = $3;
		...
		let oeff = Absyn::parse_rgneffects(SLOC(@1),$8);
		let throws = $9;
      $$=^$(Abstractdeclarator(rnew(yyr)
											 List(rnew(yyr) 
												Function_mod(rnew(yyr) 
												 WithTypes(lis,b,c,eff,po,$5,$6,
															 ieff,oeff,throws,$10)),$1.tms)));
    }
/* Cyc: new */
...
spawn_statement:
 SPAWN '(' argument_expression_list ')' 
       qual_opt_identifier
       '(' argument_expression_list ')' ';'
   { 
    let ve = Absyn::var_exp($5, LOC(@5,@5));
     $$=^$(Absyn::spawn_stmt($3,ve,$7,LOC(@1,@9))); 
   }

| SPAWN  qual_opt_identifier '(' argument_expression_list ')' ';'
   { 
     let ve = Absyn::var_exp($2, LOC(@2,@2));
      $$=^$(Absyn::spawn_stmt(NULL,ve,$4,LOC(@1,@6))); 
   }

| SPAWN  qual_opt_identifier '('  ')' ';'
   {
     let ve = Absyn::var_exp($2, LOC(@2,@2));
     $$=^$(Absyn::spawn_stmt(NULL,ve,NULL,LOC(@1,@5))); 
   }

| SPAWN '(' argument_expression_list ')' qual_opt_identifier '('  ')' ';'
   {
     let ve = Absyn::var_exp($5, LOC(@5,@5));
     $$=^$(Absyn::spawn_stmt($3,ve,NULL,LOC(@1,@8))); 
   }
;

cap_statement:
   CAP '(' argument_expression_list ')' ';'
   {  
           $$=^$(cap_stmt($3,LOC(@1,@4))); 
   }
;
