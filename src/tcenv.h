...
tenv_t tc_init();
tenv_t new_fenv_in_env(seg_t loc, tenv_t te, fndecl_t fd,type_t);
fenv_t nested_fenv(seg_t, fenv_t old_fenv, fndecl_t new_fn,type_t);
...
tenv_t new_outlives_constraint_special(tenv_t te,type_t xparent, type_t child );
	
// void calculate_throws_and_pop(seg_t loc, tenv_t te ,    datatype Tcpat::Decision @`H d );
// void add_exception(tenv_t te, type_t t );
// void push_throws_scope( tenv_t te  );
// void pop_throws_scope( tenv_t te );
 //void print_exn_scope( tenv_t te );
